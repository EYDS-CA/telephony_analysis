...
def verify_matches(existing_ios_df, new_ios_df, matches):
    """Verify match quality and show examples"""
    print("\n✅ Verifying match quality...")
    
    # Show confidence distribution
    confidences = [m['confidence'] for m in matches]
    print(f"   Average match confidence: {np.mean(confidences):.2%}")
    print(f"   Confidence range: {min(confidences):.2%} - {max(confidences):.2%}")
    
    # Show sample matches
    print("\n   📋 Sample matches for verification:")
    
    # Show 3 exact and 3 fuzzy matches
    exact_matches = [m for m in matches if m['match_type'] == 'exact'][:3]
    fuzzy_matches = [m for m in matches if m['match_type'] == 'fuzzy'][:3]
    
    for i, match in enumerate(exact_matches):
        existing = existing_ios_df.loc[match['existing_idx']]
        new = new_ios_df.iloc[match['new_idx']]
        print(f"\n   Exact Match {i+1}:")
        print(f"      Author: {existing['author']}")
        print(f"      Rating: {existing['rating']}★")
        print(f"      Date to add: {match['date_to_add']}")
        print(f"      Text preview: {str(existing.get('text', ''))[:50]}...")
    
    for i, match in enumerate(fuzzy_matches):
        existing = existing_ios_df.loc[match['existing_idx']]
        new = new_ios_df.iloc[match['new_idx']]
        print(f"\n   Fuzzy Match {i+1} (confidence: {match['confidence']:.1%}):")
        print(f"      Existing: {existing['author']} - {existing['rating']}★")
        print(f"      Matched:  {new['author']} - {new['rating']}★")
        print(f"      Date to add: {match['date_to_add']}")
...
def generate_report(existing_df, updated_df, matches, unmatched_existing, unmatched_new):
    ...
    if len(unmatched_new) > 0:
        print(f"   1. Run Claude analysis on {len(unmatched_new)} new reviews")
    print("   2. Update dashboard with refreshed data")
    print("   3. Verify iOS date-based insights")

def main():
    """Main execution"""
    print("🚀 iOS Date Updater")
    print("=" * 60)
    
    # Check arguments
    if len(sys.argv) != 3:
        print("\n❌ Usage: python ios_date_updater.py <existing_data.csv> <new_ios_data.csv>")
        print("\nExample:")
        print("  python ios_date_updater.py telecom_app_reviews_analyzed.csv ios_reviews_fresh_20250529_063833.csv")
        sys.exit(1)
    ...
    try:
        ...
        # Verify matches
        verify_matches(existing_ios_df, new_ios_df, matches)
        
        # Ask for confirmation
        print("\n🤔 Proceed with date updates? (y/n): ", end='')
        if input().lower() != 'y':
        ...
        # Add new reviews if any
        if len(unmatched_new) > 0:
            print(f"\n🤔 Add {len(unmatched_new)} new iOS reviews? (y/n): ", end='')
            if input().lower() == 'y':
                updated_df = add_new_reviews(updated_df, new_ios_df, unmatched_new)
        
        # Generate report
        generate_report(existing_df, updated_df, matches, unmatched_existing, unmatched_new)
        ...
        if 'needs_claude_analysis' in updated_df.columns:
            ...
            if len(needs_analysis) > 0:
                ...
                needs_analysis[['review_id', 'app_name', 'text']].to_csv(
                    f'reviews_needing_claude_analysis_{timestamp}.csv', 
                    index=False
                )
                print(f"\n📄 Saved list of reviews needing Claude analysis")
